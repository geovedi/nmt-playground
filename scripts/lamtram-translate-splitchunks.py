...
class Segmenter(object):
    SPLIT_AFTER = (', ', ') ', '} ', '] ', '| ', ' -- ', ': ', '; ', '. ',
                   '? ', '! ', '" ', )
    SPLIT_BEFORE = (' (', ' {', ' [', ' |', ' http://', ' https://',
                    ' ftp://', )

    def __init__(self, code_fname, separator='￭'):
        self.codes = [
            tuple(item.split()) for item in io.open(
                code_fname, encoding='utf-8')
        ]
        self.codes = dict(
            [(code, i) for (i, code) in reversed(list(enumerate(self.codes)))])
    ...
    def tokenize(self, text):
        ...
        text = re.sub(r'([\p{L}\p{N}])([\p{P}\p{S}]+) ',
                      r'\1 {}\2 '.format(self.separator), text)
        text = re.sub(r' ([\p{P}\p{S}]+)([\p{L}\p{N}])',
                      r' \1{} \2'.format(self.separator), text)
        text = re.sub(r'([\p{L}\p{N}])([\p{P}\p{S}]+)([\p{L}\p{N}])',
                      r'\1 {}\2{} \3'.format(self.separator,
                                             self.separator), text)
        text = re.sub(r' ([\p{N}]+)([\p{L}])', r' \1 \2', text)
        text = re.sub(r" {}'{} ([sSmMdD]|ll|re|ve|LL|RE|VE) ".format(
            self.separator, self.separator), r" {}'\1 ".format(self.separator),
                      text)
        text = re.sub(r"([Nn]) {}'{} ([Tt])".format(self.separator,
                                                    self.separator),
                      r" {}\1'\2 ".format(self.separator), text)
        text = re.sub(r' +', ' ', text)
        text = re.sub(r'^ ', '', text)
    ...
    def detokenize(self, text):
        ...
        text = re.sub(r'$', ' ', text)
        text = re.sub(r" {}'([sSmMdD]|ll|re|ve|LL|RE|VE) ".format(
            self.separator), r"'\1 ", text)
        text = re.sub(r" {}([Nn])'([Tt]) ".format(self.separator), r"\1'\2 ",
                      text)
    ...
    def punct_split(self, word):
        word = re.sub(r'^([\p{P}]+)([\p{L}\p{N}]+)',
                      r'\1{} \2'.format(self.separator), word)
        word = re.sub(r'([\p{L}\p{N}]+)([\p{P}]+)$',
                      r'\1 {}\2'.format(self.separator), word)
        return word
...
def main(model,
    ...
    lamtram_cmd = '''
        lamtram
            ...
            --models_in encatt={1}
            --operation gen
    '''.format(dynet_mem, model).split()

    proc = subprocess.Popen(
        lamtram_cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE)

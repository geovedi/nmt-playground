...
class BPE(object):
    def __init__(self, codes, separator='￭', vocab=None, glossaries=None):
        ...
        self.glossaries = glossaries if glossaries else ['|||', '<unk>']
        self.cache = {}
    ...
    def segment(self, sentence):
        ...
        for word in sentence.split():
            if self.separator in word or word in self.glossaries:
                output.append(word)
                continue

            new_word = [
                out
                for segment in self._isolate_glossaries(word) for out in
                encode(segment, self.bpe_codes, self.bpe_codes_reverse,
                       self.vocab, self.separator, self.version, self.cache,
                       self.glossaries)
            ]
...
def create_parser():
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="PATH",
        help=("Vocabulary file (built with get_vocab.py). "
              "If provided, this script reverts any merge operations that "
              "produce an OOV."))
    parser.add_argument(
        ...
        default=2,
        metavar="INT",
        help=("Vocabulary threshold. If vocabulary is provided, any word with "
              "frequency < threshold will be treated as OOV"))
    parser.add_argument(
        '--glossaries',
        '-g',
        type=str,
...
def encode(orig,
           ...
           cache,
           glossaries=None):
    """Encode word based on list of BPE merge operations, which are applied
    consecutively.
    """
...
def recursive_split(segment, bpe_codes, vocab, separator, final=False):
    ...
    if (final and right in vocab) or (not final and
                                      right + separator in vocab):
        yield right
    else:
...
def read_vocabulary(vocab_file, threshold=None):
    ...
    if vocab_file.name.endswith('.json'):
        import json
        return set(json.load(vocab_file).keys())
...
def isolate_glossary(word, glossary):
    ...
    For example, if 'USA' is the glossary and '1934USABUSA' the word, the
    return value is:
        ['1934', 'USA', 'B', 'USA']
    """
    if word == glossary or glossary not in word:
        ...
        segments = [
            segment.strip()
            for split in splits[:-1] for segment in [split, glossary]
            if segment != ''
        ]
        return segments + [splits[-1].strip()] if splits[
            -1] != '' else segments
